...
- It computes a 3x3 homography H from a reference (projector pattern) and a camera capture,
  then sends H to the projector app over its H ZMQ endpoint (REP at tcp://127.0.0.1:5560).
...
def send_h_over_zmq(H: np.ndarray, endpoint: str = "tcp://127.0.0.1:5560", timeout_ms: int = 500):
    ...
    D-asift-1fix: delegate to the L3-audited
    `core.projector._send_homography_inline` helper. This is the
    FOURTH inline-ZMQ site in the codebase — D-cam-3 + D-l4-1 + D-l4-8
    were the other three, unified in commit 8b0f299. Same audited
    contract: RCVTIMEO + WARNING-level log on no-ACK + try/finally
    socket cleanup + bool return.

    Returns the bool from the helper. Callers that ignore the return
    keep working unchanged (the helper logs failures at WARNING).
    """
    if H.shape != (3, 3):
        raise ValueError("H must be 3x3")

    # Resolve the audited helper. It lives under
    # STIMViewer_CRISPI/CS/core/projector.py.
    # asift_calibration.py is in ZMQ_sender_mask/, so we need the CS
    # path on sys.path.
    try:
        import sys as _sys
        from pathlib import Path as _Path
        _cs = (_Path(__file__).resolve().parent.parent
               / "STIMViewer_CRISPI" / "CS")
        ...
        # Fall back to inline ZMQ if the audited helper isn't reachable
        # (e.g., running asift_calibration.py from a deployment that
        # doesn't include the CS package). Preserves backward compat.
        print(f"[asift] audited helper unavailable ({_import_e}); "
              f"using inline ZMQ fallback")
        return _send_h_over_zmq_inline_fallback(H, endpoint, timeout_ms)

    return _send_homography_inline(H, endpoint, rcvtimeo_ms=timeout_ms)


def _send_h_over_zmq_inline_fallback(
    H: np.ndarray, endpoint: str, timeout_ms: int,
) -> bool:
    """Backward-compat inline-ZMQ fallback when audited helper missing.

    Pre-D-asift-1, this WAS the implementation. Kept for the (rare)
    case where the CS package isn't on sys.path. Returns True/False
    matching the audited helper's contract.
    """
    _require_zmq()
    sock = None
    try:
