...
class Display(QtWidgets.QGraphicsView):
    ...
    # Emits (x, y, intensity_str) when pixel probe clicks on the image
    pixel_probe_signal = QtCore.pyqtSignal(int, int, str)

    def __init__(self, parent=None):
        super().__init__(parent)
        self._pixel_probe_enabled = False
    ...
    def on_image_received(self, qimg):
        # DEBUG (off unless STIM_FRAME_DEBUG=1): count display-side frames.
        # Throttled to ~1/sec at 30 fps. If iface emits but this never logs,
        # the QueuedConnection isn't actually delivering.
        import os
        if os.environ.get("STIM_FRAME_DEBUG") == "1":
            self._disp_frame_count = getattr(self, "_disp_frame_count", 0) + 1
            if self._disp_frame_count % 30 == 1:
                kind = type(qimg).__name__
                is_null = qimg.isNull() if isinstance(qimg, QtGui.QImage) else "n/a"
                size = f"{qimg.width()}x{qimg.height()}" if isinstance(qimg, QtGui.QImage) else "n/a"
                visible = "yes" if self.isVisible() else "NO"
                print(f"[FRAME-DEBUG disp] on_image_received #{self._disp_frame_count} "
                      f"type={kind} null={is_null} {size} widget_visible={visible}")
    ...
    def mousePressEvent(self, ev: QtGui.QMouseEvent):
        if ev.button() == QtCore.Qt.LeftButton and self._pixel_probe_enabled:
            # Pixel probe: report coordinates + intensity under cursor
            self._do_pixel_probe(ev.pos())
            ev.accept()
    ...
    def _do_pixel_probe(self, view_pos):
        """Read pixel coordinates and intensity at the clicked position."""
        try:
            scene_pos = self.mapToScene(view_pos)
            x, y = int(scene_pos.x()), int(scene_pos.y())
            pm = self._img_item.pixmap()
            if pm.isNull():
                return
            img = pm.toImage()
            if x < 0 or y < 0 or x >= img.width() or y >= img.height():
                self.pixel_probe_signal.emit(x, y, "out of bounds")
                return
            ...
            if r == g == b:
                ...
                info = f"R={r} G={g} B={b}"
            self.pixel_probe_signal.emit(x, y, info)
        except Exception as e:
            print(f"Pixel probe error: {e}")
