...
def view_exported_traces(traces_file="live_traces.npy", roi_info_file="roiprint_export.npz"):
    ...
    raw = np.load(traces_file, allow_pickle=False)
    # live_traces.npy stores a structured array with named fields; convert to dict
    if raw.dtype.names:
        traces = {name: raw[name] for name in raw.dtype.names}
    else:
        raise ValueError(
            f"{traces_file} has unsupported format (dtype={raw.dtype}). "
            "Re-export traces using the current pipeline to get a structured array."
        )
