...
def setup_opengl_safety():
    ...
    try:
        ...
        # Reduce camera/buffer pressure by default (can be overridden by user env)
        os.environ.setdefault("STIM_PEAK_BUFFERS", "8")
        os.environ.setdefault("STIM_CAMERA_FPS", "30")
        # Monochrome sensors: prefer MONO8 by default (user can override via env or GUI)
        os.environ.setdefault("STIM_PIXEL_FORMAT", "MONO8")
...
class DisplayManager:
    ...
    def _x_is_available(display: str) -> bool:
        try:
            ...
            # nosec B108: /tmp/.X11-unix/X<N> is the X.org/freedesktop
            # standard X server socket path. This is a read-only `.exists()`
            # check — we're not creating or writing files there. The path is
            # not user-controlled; idx is derived from the DISPLAY env var
            # which the OS provides.
            return Path(f"/tmp/.X11-unix/X{idx}").exists()  # nosec B108
        except Exception:
            return False
...
class ApplicationManager:
    def __init__(self):
        ...
        # Lazily create heavy subsystems only if enabled
        self.zero_mq_manager = None
    ...
    def _apply_env_overrides(self):
        """Allow disabling heavy subsystems via environment. Safe-mode by default."""
        ...
        # Global safe mode (default to ON to avoid crashes on constrained systems)
        safe = _env_bool("STIMVIEWER_SAFE", True)

        # Individual toggles (default derived from safe)
        self.config.enable_zero_mq = _env_bool("STIMVIEWER_ENABLE_ZMQ", not safe) and _env_bool("STIMVIEWER_ZMQ", not safe)
        self.config.enable_performance_monitoring = _env_bool("STIMVIEWER_ENABLE_PERF", not safe) and _env_bool("STIMVIEWER_PERF", not safe)
        self.config.enable_multiprocessing = _env_bool("STIMVIEWER_ENABLE_MP", not safe) and _env_bool("STIMVIEWER_MP", not safe)
        self.config.safe_process_pool_on_jetson = _env_bool("STIMVIEWER_SAFE_MP_JETSON", False)
        self.config.enable_jetson_optimizations = _env_bool("STIMVIEWER_JETSON_OPT", not safe and os.path.exists("/etc/nv_tegra_release"))
        # If safe, also dial back thread counts a bit
        if safe:
            self.config.io_threads = max(1, min(self.config.io_threads, 2))
            self.config.camera_threads = max(1, min(self.config.camera_threads, 1))
    ...
    def initialize(self) -> bool:
        try:
            ...
            if self.config.enable_zero_mq and self.zero_mq_manager:
            ...
            if self.config.enable_zero_mq:
                if self.zero_mq_manager is None:
            ...
            if self.config.enable_zero_mq and self.zero_mq_manager and self.performance_monitor:
                def _pub_perf():
                    m = self.performance_monitor.get_current_metrics().__dict__
                    self.zero_mq_manager.publish_message("perf_metrics", m)
                from PyQt5.QtCore import QTimer, QObject
                parent = self.app if getattr(self, "app", None) is not None else None
                self._perf_pub_timer = QTimer(parent)
                self._perf_pub_timer.timeout.connect(_pub_perf)
                self._perf_pub_timer.start(2000)
    ...
    def _create_qt_application(self):
        try:
            ...
            # Heartbeat so the Python interpreter periodically regains control
            # while Qt's C++ event loop runs. Without it, pending Unix signals
            # (SIGINT from Ctrl+C, SIGTERM) are never delivered to the handlers
            # in _setup_signal_handlers, so the process can't be killed from the
            # launching terminal. A no-op 200 ms tick is imperceptible and makes
            # Ctrl+C work regardless of whether perf-monitoring/ZMQ are enabled.
            self._signal_wakeup_timer = QTimer()
            self._signal_wakeup_timer.timeout.connect(lambda: None)
            self._signal_wakeup_timer.start(200)
        except Exception as e:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    print("WARN", "Continuing without IDS Peak camera support")
                    self._ids_initialized = False
                    return
                time.sleep(1.0)
