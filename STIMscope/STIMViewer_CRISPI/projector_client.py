...
class ProjectorClient:
    ...
    The projector engine listens on tcp://127.0.0.1:5558 and expects multipart messages:
    ...
    def send_rgb(self, img_rgb: np.ndarray, frame_id: Optional[int] = None, immediate: bool = True, visible_overlay: Optional[bool] = None) -> None:
        """Send a packed-RGB frame (H, W, 3) uint8 to the projector engine.

        Matches the mask sender's --composite-rgb / --temporal-alternate byte
        layout: H*W*3 raw bytes. The engine auto-detects 1ch vs 3ch by size.
        """
        if not isinstance(img_rgb, np.ndarray):
            raise TypeError("img_rgb must be np.ndarray")
        if img_rgb.ndim != 3 or img_rgb.shape[2] != 3:
            raise ValueError("img_rgb must be shape (H, W, 3)")
        if img_rgb.shape[:2] != (self.height, self.width):
            import cv2
            img_rgb = cv2.resize(img_rgb, (self.width, self.height), interpolation=cv2.INTER_NEAREST)
